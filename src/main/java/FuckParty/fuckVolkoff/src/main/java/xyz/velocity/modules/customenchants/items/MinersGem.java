...
@EnchantItem
public class MinersGem extends AbstractEnchantItem {
    ...
    public MinersGem() {
        ...
        EnchantItemSave item = new EnchantItemSave(true, "miners_gem", "&b&lMiners Gem", Arrays.asList("Enchant your pickaxe to efficiency 10"), Arrays.asList(""), "DIAMOND", 0, true, this.extraInfo());
    ...
    public <T extends Event> void runTask(T event) {
        ...
        String compound = "velocity_enchantItem_minersGem";
        ...
        if (enchantItemCompound.getBoolean("minersGem")) return;
        if (!nbtItem.getItem().getType().name().endsWith("_PICKAXE")) return;

        enchantItemCompound.setBoolean("minersGem", true);
        EnchantItems.getInstance().addEnchantsToItem(nbtItem.getItem(), Enchantment.DIG_SPEED, 10, false);
