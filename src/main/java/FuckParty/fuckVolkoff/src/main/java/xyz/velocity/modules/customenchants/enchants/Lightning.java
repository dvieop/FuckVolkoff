...
@Enchant
public class Lightning extends AbstractEnchant {
    ...
    public Lightning() {
        ...
        EnchantSave enchant = new EnchantSave(true, "Lightning", "&aLightning", Arrays.asList("&7Smite your opponents and do damage"), "SWORD", "LIGHTNING:<level>", 2, 3, 20, 1, false, this.extraInfo());
    ...
    public <T extends Event> void runTask(T event) {
        ...
        if (chance < enchantInformation.second) {
            ...
            violated.getWorld().spawnEntity(loc, EntityType.LIGHTNING);
            violated.getWorld().strikeLightningEffect(loc);
