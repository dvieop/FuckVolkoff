...
@EnchantItem
public class DepthStrider extends AbstractEnchantItem {
    ...
    public DepthStrider() {
        ...
        EnchantItemSave item = new EnchantItemSave(true, "depth_strider", "&9&lScuba Shard &7(Depth Strider)", Arrays.asList("Enhance your boots with depth strider"), Arrays.asList(""), "PRISMARINE_SHARD", 0, true, this.extraInfo());
    ...
    public <T extends Event> void runTask(T event) {
        ...
        String compound = "velocity_enchantItem_depthStrider";
        ...
        if (enchantItemCompound.getBoolean("depthStrider")) return;
        if (!nbtItem.getItem().getType().name().endsWith("_BOOTS")) return;

        enchantItemCompound.setBoolean("depthStrider", true);
        EnchantItems.getInstance().addEnchantsToItem(nbtItem.getItem(), Enchantment.DEPTH_STRIDER, 3, false);
